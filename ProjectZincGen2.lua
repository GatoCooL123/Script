...
-- Point this at wherever you host the script.
local SCRIPT_URL = "https://raw.githubusercontent.com/GatoCooL123/Script/main/IQObbyScriptv0.5.lua"
...
local function PurchaseItem(itemName)
    ...
    if not remote then
        window:Notify({ title = "Failed", content = "Purchasing remote not found" })
        return false
    end
...
local function BuildCategoryTab(tabName, sectionName, items)
    ...
    tab:CreateButton({
        ...
        callback = function()
            if #selected == 0 then
                window:Notify({ title = "Nothing selected", content = "Pick at least one item first." })
                return
            end
            local count = #selected
            window:Notify({ title = "Spawning", content = count .. " item(s)..." })
            task.spawn(function()
                for _, name in ipairs(selected) do
                    PurchaseItem(name)
                    task.wait(0.1)
                end
                window:Notify({ title = "Done", content = "Spawned " .. count .. " item(s)." })
            end)
        end,
    })
    ...
    tab:CreateButton({
        ...
        callback = function()
            window:Notify({ title = "Spawning", content = "All " .. #items .. " items..." })
            task.spawn(function()
                for _, name in ipairs(items) do
                    PurchaseItem(name)
                    task.wait(0.1)
                end
                window:Notify({ title = "Done", content = "Spawned " .. #items .. " items." })
            end)
        end,
    })
...
harmfulTab:CreateButton({
    ...
    callback = function()
        ...
        if foundTool then
            ...
            window:Notify({ title = "Slap", content = "Found and cloned: " .. foundTool.Name })
        else
            PurchaseItem("Slap")
            window:Notify({ title = "Slap", content = "No player had it, requested by name." })
        end
    end,
})
...
customTab:CreateButton({
    ...
    callback = function()
        ...
        if not char then
            window:Notify({ title = "Failed", content = "No character loaded." })
            return
        end
        ...
        window:Notify({ title = "IQ Omega", content = "Setup complete." })
    end,
})
...
customTab:CreateButton({
    ...
    callback = function()
        ...
        window:Notify({ title = "IQ Unreal", content = "Setup complete." })
    end,
})
...
manualTab:CreateInput({
    ...
    callback = function(text)
        if text and #text > 0 then
            PurchaseItem(text)
            window:Notify({ title = "Requested", content = text })
        end
    end,
})
...
local function hopToNext(state)
    ...
    for _ = 1, 10 do
        ...
        if not data or not data.data then
            window:Notify({ title = "Find Player", content = "Could not list servers." })
            return false
        end
        ...
        for _, server in ipairs(data.data) do
            ...
            then
                ...
                if typeof(queueonteleport) ~= "function" then
                    window:Notify({
                        title = "Find Player",
                        content = "Executor lacks queueonteleport, cannot resume.",
                    })
                    clearState()
                    return false
                end
        ...
        if cursor == "" then
            window:Notify({ title = "Find Player", content = "Searched every server. Not found." })
            clearState()
            return false
        end
    end
...
findTab:CreateButton({
    ...
    callback = function()
        if targetName == "" then
            window:Notify({ title = "Find Player", content = "Enter a username first." })
            return
        end
        ...
        if not hasFileSupport() then
            window:Notify({
                title = "Find Player",
                content = "Executor lacks file support, cannot resume after hop.",
            })
            return
        end
        ...
        if here then
            window:Notify({ title = "Found", content = targetName .. " is in this server." })
            return
        end
        ...
        saveState(state)
        window:Notify({ title = "Find Player", content = "Hopping..." })
        hopToNext(state)
    end,
})
...
findTab:CreateButton({
    ...
    callback = function()
        clearState()
        window:Notify({ title = "Find Player", content = "Search stopped." })
    end,
})
...
task.spawn(function()
    ...
    if found then
        clearState()
        window:Notify({ title = "Found", content = state.target .. " is in this server." })
        return
    end

    window:Notify({ title = "Find Player", content = "Not here, hopping..." })
    task.wait(1)
    hopToNext(state)
end)
